...
describe("Move ETH token tests", function () {
    ...
    it("Move assets from contract to cosmos", async () => {
        ...
        const amount = Utils.imvToAimv(100) //10 000 000 000 000 000 000 000
        ...
        const balanceBefore = balanceBeforeResp.data.balance?.amount
        const client = new ImversedBankClient(walletStorageMock, mockConfig)
        const res = await client.moveAssetsFromEthToCosmos(amount, ethContractAddress)
        ...
        // @ts-ignore
        expect((balanceAfter - parseInt(amount))).eq(parseInt(balanceBefore))

        // return back
        //await client.moveAssetsFromCosmosToEth(amount, denom)
    })
})
